...
CheckInfo::CheckInfo(const Position& pos) {
	...
	checkSq[KNIGHT] = knight_attacks_from(ksq, pos.pieces(), pos.piecesl90());
...
Bitboard Position::attackers_to(Square s) const {
	...
	return (pawn_attacks_from(BLACK, s) & pieces(BLACK, PAWN))
		...
		| (cannon_control_bb(s, occupied, occupied_rl90) & (pieces(WHITE, CANNON) | pieces(BLACK, CANNON)))
		| (rook_attacks_bb(s, occupied, occupied_rl90) & (pieces(WHITE, ROOK) | pieces(BLACK, ROOK)))
		| (knight_attacks_from(s, occupied, occupied_rl90) & (pieces(WHITE, KNIGHT) | pieces(BLACK, KNIGHT)))
...
Bitboard Position::checkers(Color checker, Square ksq) const {
	return pawn_attacks_from(checker, ksq) & pieces(checker, PAWN)
		| cannon_control_bb(ksq, occupied, occupied_rl90) & pieces(checker, CANNON)
		| rook_attacks_bb(ksq, occupied, occupied_rl90) &  pieces(checker, ROOK)
		| knight_attacks_from(ksq, occupied, occupied_rl90) & pieces(checker, KNIGHT);		 
}
