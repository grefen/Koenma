...
Bitboard KnightLegImagic[SQUARE_NB];
Bitboard KnightEyeImagic[SQUARE_NB];
...
Bitboard knight_attack_to(Square sq, Bitboard leg) {
...
Bitboard knight_attack_from(Square sq, Bitboard eye) {
...
void init_knightimagic(){
	...
	for (Square s = SQ_A0; s <= SQ_I9; ++s)
	{
		...
		do
		{
			...
			{
				occupancy[size] = b;
				reference[size] = knight_attack_to(s, b);

				size++;	

				b.bb[0] = (b.bb[0] - KnightLeg[s].bb[0])&KnightLeg[s].bb[0];

			} while (b.bb[0]);
			b.bb[1] = (b.bb[1] - KnightLeg[s].bb[1])&KnightLeg[s].bb[1];
		...
		do {
			//do
			//{
				KnightLegImagic[s] = rk.magic_bitboard(booster);

			//} while (popcount64(((KnightLegImagic[s].bb[0] * KnightLegImagic[s].bb[0]) ^ (KnightLegImagic[s].bb[1] * KnightLegImagic[s].bb[1])) >> 54) < 5);

			
			std::memset(KnightImagicAttackTo[s], 0, size * sizeof(Bitboard));
			...
			for (i = 0; i < size; ++i)
			{
				Bitboard& attack = KnightImagicAttackTo[s][knight_imagic_index(s, occupancy[i], KnightLegImagic[s])];
...
void init_knighteyeimagic() {
	...
	for (Square s = SQ_A0; s <1/* SQ_I9*/; ++s)
	{
		...
		do
		{
			...
			{
				occupancy[size] = b;
				reference[size] = knight_attack_from(s, b);					

				size++;

				b.bb[0] = (b.bb[0] - KnightEye[s].bb[0])&KnightEye[s].bb[0];

			} while (b.bb[0]);
			b.bb[1] = (b.bb[1] - KnightEye[s].bb[1])&KnightEye[s].bb[1];
		...
		do {
			//do
			//{
			KnightEyeImagic[s] = rk.magic_bitboard(booster);

			//} while (popcount64(((KnightLegImagic[s].bb[0] * KnightLegImagic[s].bb[0]) ^ (KnightLegImagic[s].bb[1] * KnightLegImagic[s].bb[1])) >> 54) < 5);


			std::memset(KnightImagicAttackFrom[s], 0, size * sizeof(Bitboard));
			...
			for (i = 0; i < size; ++i)
			{
				Bitboard& attack = KnightImagicAttackFrom[s][knight_imagiceye_index(s, occupancy[i], KnightEyeImagic[s])];
