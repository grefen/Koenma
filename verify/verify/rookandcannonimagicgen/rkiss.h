...
class RKISS {
  ...
  uint32_t rotate_L32(uint32_t x, unsigned k) const {
	  return (x << k) | (x >> (32 - k));
  }
...
public:
  ...
  template<typename T> T magic_rand32(int s) {
	  return rotate_L32(rotate_L32(rand<T>(), (s >> 0) & 0x3F) & rand<T>()
		  , (s >> 6) & 0x3F) & rand<T>();
  }

  uint96_t magic_bitboard(int s){
	  uint96_t t;
	  t.bb[0] = magic_rand32<uint32_t>(s);
	  t.bb[1] = magic_rand32<uint32_t>(s);
	  t.bb[2] = magic_rand32<uint32_t>(s);
	  return t;
  }
};
