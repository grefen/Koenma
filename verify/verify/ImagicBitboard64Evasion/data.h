...
extern uint64_t KnightLegImagic[SQUARE_NB];
extern uint64_t KnightEyeImagic[SQUARE_NB];

extern Bitboard BishopLeg[SQUARE_NB];
extern Bitboard BishopImagicAttack[SQUARE_NB][16];
extern uint64_t BishopLegImagic[SQUARE_NB];
...
inline unsigned supercannon_magic_index(Square s, const Bitboard& occ) {
...
template<KnightBlock Block>
inline 	unsigned knight_imagic_index(Square s, const Bitboard& occ) {

	Bitboard* const Masks  = Block == KNIGHT_LEG ?  KnightLeg : KnightEye;
	uint64_t* const Magics = Block == KNIGHT_LEG ?  KnightLegImagic : KnightEyeImagic;

	Bitboard t = Masks[s] & occ;
	return ((t.bb[0] << 18 ^ t.bb[1] << 18)*Magics[s]) >> 60;
}

inline unsigned bishop_imagic_index(Square s, const Bitboard& occ) {
	Bitboard t = BishopLeg[s] & occ;
	return ((t.bb[0] << 18 ^ t.bb[1] << 18)*BishopLegImagic[s]) >> 60;
}
...
{
	return RAttacks[s][slider_magic_index<ROOK>(s, occ)];
}
inline Bitboard cannon_control_bb(Square s, Bitboard occ)
{
	return CannonAttacks[s][slider_magic_index<CANNON>(s, occ)];
}
inline Bitboard cannon_super_control_bb(Square s, Bitboard occ)
{
	return SuperCannonAttacks[s][supercannon_magic_index(s, occ)];
}
inline Bitboard knight_attacks_bb(Square s, Bitboard occ)
{
	return KnightImagicAttackTo[s][knight_imagic_index<KNIGHT_LEG>(s, occ)];
}
inline Bitboard knight_attacks_from(Square s, Bitboard occ)
{
    return KnightImagicAttackFrom[s][knight_imagic_index<KNIGHT_EYE>(s, occ)];
}
inline Bitboard bishop_attacks_bb(Square s,  Bitboard occ)
{
	return BishopImagicAttack[s][bishop_imagic_index(s, occ)];
}
