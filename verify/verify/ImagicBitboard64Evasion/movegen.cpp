...
bool move_is_legal(Position& pos, ExtMove move)
{
	...
	if((RookAttackMask[ksq]& cannons) &&(cannon_control_bb(ksq, occ) & cannons)) return false;
	if((RookAttackMask[ksq]& rooks) && (rook_attacks_bb(ksq,occ)& rooks) ) return false;
	if((KnightAttackMask[ksq]&knights) && (knight_attacks_from(ksq, occ)&knights) ) return false;
	if((pawn_attacks_from(~us,ksq) & pawns) ) return false;

	if((RookAttackMask[ksq]& pos.king_square(~us)) && (rook_attacks_bb(ksq,occ)& pos.king_square(~us))) return false;//对脸 
...
void gen_evasion_moves(Position& pos, MoveList* mlist)
{
	...
	{		
		...
		case ROOK:
			...
			break;
		case CANNON:
			sliderAttacks |= cannon_control_bb(checksq, occ);
			sliderAttacks |= cannon_super_control_bb(checksq, occ);
			break;
		default:
			break;
		}
