...
struct CheckInfo {
  ...
  Bitboard dcCandidates;  //可以将对方king的子
  Bitboard dcCannonCandidates;//充当炮架的位置，我方cannon与对方king face to face
  Bitboard pinned;	 //我方被牵制的子
  Bitboard checkSquares[PIECE_TYPE_NB];	 //下一步我方可直接将军的位置，
  Square   ksq;
};
...
inline Bitboard Position::attacks_from(Square s) const {
	...
	if(Pt == ROOK )	 return rook_attacks_bb(s, byTypeBB[ALL_PIECES]);
		...
		assert(false); 
		return Bitboard();//PAWN need color
	}
}
