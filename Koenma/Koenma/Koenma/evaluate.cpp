...
namespace {
  ...
  Score evaluate_threats(const Position& pos, const EvalInfo& ei) {
    ...
    if (weak)
    {
        ...
        safeThreats = (shift_bb(b, Right) | shift_bb(b, Left)) & weak;
    ...
    b = shift_bb(b | (shift_bb(b & TRank2BB, Up) & ~pos.pieces()), Up);

    b &=  ~pos.pieces()
        & ~ei.attackedBy[Them][PAWN]
        & (ei.attackedBy[Us][ALL_PIECES] | ~ei.attackedBy[Them][ALL_PIECES]);

    b =  (shift_bb(b, Left) | shift_bb(b, Right))
       &  pos.pieces(Them)
