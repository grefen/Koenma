...
uint64_t Search::perft(Position& pos, Depth depth) {
  ...
  for (const auto& m : MoveList<LEGAL>(pos))
  {
      ...
      if (Root)
          sync_cout << UCI::move(m, false) << ": " << cnt << sync_endl;
  }
  return nodes;
}
...
void MainThread::search() {
  ...
  else
  {
	  ...
	  //CheckInfo ci(rootPos);
	  //StateInfo st;
	  //std::vector<Move> tempMoves;
	  //for (const RootMove& m : rootMoves) {
		 // rootPos.do_move(m.pv[0], st, rootPos.gives_check(m.pv[0], ci));
		 // if (rootPos.is_repeat()) {
			//  tempMoves.push_back(m.pv[0]);
		 // }
		 // rootPos.undo_move(m.pv[0]);
	  //}
	  //Search::RootMoveVector::iterator it = rootMoves.begin();
	  //for (; it != rootMoves.end();) {
		 // if (std::find(tempMoves.begin(), tempMoves.end(), it->pv[0]) != tempMoves.end())
			//  it = rootMoves.erase(it);
		 // else
			//  ++it;
	  //}
  ...
  if(bestThread->rootMoves[0].pv[0] != MOVE_NONE)
      if (bestThread->rootMoves[0].pv.size() > 1 || bestThread->rootMoves[0].extract_ponder_from_tt(rootPos))
          std::cout << " ponder " << UCI::move(bestThread->rootMoves[0].pv[1], false);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
	...
	assert(pos.checkers() == pos.get_checkers(~pos.side_to_move(), pos.square<KING>(pos.side_to_move())));
	assert(!pos.get_checkers(pos.side_to_move(), pos.square<KING>(~pos.side_to_move())));
                            ...
                            : (tte->bound() & BOUND_UPPER)))
    {
        ...
        if (ttValue >= beta && ttMove && !pos.capture(ttMove))
...
moves_loop: // When in check search starts from here
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
      ...
      ss->currentMove = move;
	  assert(pos.checkers() == pos.in_check(pos.side_to_move()));
    ...
    */
    ...
    else if (bestMove && !pos.capture(bestMove))
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
	...
	assert(InCheck == pos.in_check(pos.side_to_move()));
